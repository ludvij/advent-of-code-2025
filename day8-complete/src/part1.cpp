...
u64 do_program(const char* path, u64 connections)
{
    ...
    return stdr::fold_left(
        circuits |
            stdv::take(3) |
            stdv::transform(Lud::as_size<std::unordered_set<Point3D>>()),
        1UL,
        std::multiplies<>()
    );
}
...
{
    Lud::assert::eq(do_program(INPUT_PATH "test.txt", 10), 40U);

    u64 result = do_program(INPUT_PATH "input.txt", 1000);
