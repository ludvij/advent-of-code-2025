...
u64 do_program(const char* path)
{
    ...
    return cached_dfs(graph, "svr", "fft") *
           cached_dfs(graph, "fft", "dac") *
           cached_dfs(graph, "dac", "out");
    //     cached_dfs(graph, "svr", "dac") *
    //     cached_dfs(graph, "dac", "fft") *
    //     cached_dfs(graph, "fft", "out");
}

int main(int argc, char** argv)
{
    Lud::assert::eq(do_program(INPUT_PATH "test_part2.txt"), 2U);
